...
router.put("/scoring/rounds/:id/holes/:holeNum", async (req, res) => {
  try {
    ...
    // Betterball / opponent / marker player scores — apply max-score cap per player
    if (Array.isArray(players)) {
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        if (pRawGross != null && !p.isNr) {
          // Map player index to the stored playing handicap for that slot
          const pHcp = isBBFmt
            ? (i === 0 ? Number(partner_playing_hcp  ?? 0)
             : i === 1 ? Number(opponent_playing_hcp  ?? 0)
             :           Number(opponent2_playing_hcp ?? 0))
            : (i === 0 ? Number(opponent_playing_hcp  ?? 0)
             :           Number(opponent2_playing_hcp ?? 0));
...
router.patch("/scoring/rounds/:id/handicaps", async (req, res) => {
  try {
    ...
    // Recalculate scoring_player_holes when any opponent/partner handicap changes
    const anyOppHcpChanged =
      newOppHcp     !== Number(round.opponent_playing_hcp)  ||
      newPartnerHcp !== Number(round.partner_playing_hcp)   ||
      newOpp2Hcp    !== Number(round.opponent2_playing_hcp);
    if (anyOppHcpChanged) {
      const BB_FMTS = ["betterball_match_play","betterball_gross_match_play","fourball_stableford_match_play"];
      const isBBFmt = BB_FMTS.includes(fmt);
      const phRows = await query<any>(`
      ...
      for (const ph of phRows) {
        const pHcp = isBBFmt
          ? (ph.player_index === 0 ? Number(newPartnerHcp ?? 0)
           : ph.player_index === 1 ? Number(newOppHcp     ?? 0)
           :                         Number(newOpp2Hcp    ?? 0))
          : (ph.player_index === 0 ? Number(newOppHcp  ?? 0)
           :                         Number(newOpp2Hcp ?? 0));
