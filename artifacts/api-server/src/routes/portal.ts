...
// Recalculate an outstanding counter-booking invoice to pull in any newly added bookings
router.post("/portal/invoices/:id/recalculate", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  const club = getClub(req);
  const invoiceId = parseInt(req.params["id"] ?? "", 10);
  if (!invoiceId) { res.status(400).json({ message: "Invalid invoice id" }); return; }

  const inv = await row<any>(
    `SELECT id, invoice_ref, invoice_type, status, total_amount FROM club_invoices WHERE id = ? AND club_id = ?`,
    [invoiceId, club.id]
  );
  if (!inv) { res.status(404).json({ message: "Invoice not found" }); return; }
  if (inv.invoice_type !== "counter_bookings") { res.status(400).json({ message: "Only counter booking invoices can be recalculated" }); return; }
  if (inv.status === "paid") { res.status(400).json({ message: "Cannot recalculate a paid invoice" }); return; }

  // Fetch all bookings already on this invoice + any new unbilled ones
  const allBookings = await query<any>(
    `SELECT b.id, b.booking_ref, b.players, b.created_at,
            COALESCE(u.name, b.guest_name, 'Guest') AS guest_name,
            pts.date, pts.tee_time AS time
     FROM bookings b
     ...
     WHERE pts.club_id = ?
       ...
       AND (b.counter_invoice_id = ? OR b.counter_invoice_id IS NULL)
     ORDER BY pts.date, pts.tee_time`,
    [club.id, invoiceId]
  );
  if (allBookings.length === 0) { res.status(400).json({ message: "No bookings to include" }); return; }

  const feeSetting = await row<any>("SELECT setting_value FROM platform_settings WHERE setting_key = 'platform_fee_flat'");
  const feePerSlot = feeSetting ? parseFloat(feeSetting.setting_value) : 10;
  const totalSlots  = allBookings.reduce((s: number, b: any) => s + Number(b.players ?? 1), 0);
  const totalAmount = Math.round(feePerSlot * totalSlots * 100) / 100;
  const vatAmount   = Math.round(totalAmount * 15 / 115 * 100) / 100;
  const description = `${totalSlots} player slot${totalSlots !== 1 ? "s" : ""} (${allBookings.length} booking${allBookings.length !== 1 ? "s" : ""}) @ R${feePerSlot.toFixed(2)}/slot — TapIn platform fee (incl. 15% VAT)`;
  const lineItems = allBookings.map((b: any) => ({
    booking_ref: b.booking_ref,
    guest_name:  b.guest_name,
    date:        String(b.date).slice(0, 10),
    time:        String(b.time).slice(0, 5),
  ...
  // Link any unbilled bookings to this invoice
  const newIds: number[] = allBookings.filter((b: any) => !b.counter_invoice_id || b.counter_invoice_id !== invoiceId).map((b: any) => b.id);
  if (newIds.length > 0) {
    const ph = newIds.map(() => "?").join(",");
    await exec(`UPDATE bookings SET counter_invoice_id = ? WHERE id IN (${ph})`, [invoiceId, ...newIds]);
  }

  // Refresh Stitch payment link with new amount
  const host = process.env["REPLIT_DEV_DOMAIN"] ? `https://${process.env["REPLIT_DEV_DOMAIN"]}` : "https://tapingolf.co.za";
  let paymentUrl: string | null = null;
  try {
    const payment = await createStitchPayment({
    ...
    await exec(
      `UPDATE club_invoices SET description = ?, total_rounds = ?, vat_amount = ?, total_amount = ?,
         line_items = ?::jsonb, stitch_payment_id = ?, stitch_payment_url = ? WHERE id = ?`,
      [description, totalSlots, vatAmount, totalAmount, JSON.stringify(lineItems), payment.id, payment.url, invoiceId]
    );
    paymentUrl = payment.url;
  } catch {
    await exec(
      `UPDATE club_invoices SET description = ?, total_rounds = ?, vat_amount = ?, total_amount = ?, line_items = ?::jsonb WHERE id = ?`,
      [description, totalSlots, vatAmount, totalAmount, JSON.stringify(lineItems), invoiceId]
    );
  }

  res.json({ id: invoiceId, total_slots: totalSlots, total_bookings: allBookings.length, total_amount: totalAmount, payment_url: paymentUrl, added: newIds.length });
});
