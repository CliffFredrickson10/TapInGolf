...
router.get("/portal/dashboard", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const from: string = typeof req.query.from === "string" && req.query.from ? req.query.from : today;
  const to:   string = typeof req.query.to   === "string" && req.query.to   ? req.query.to   : today;

  const [teeTimes, bookings, reviews, members, events, revenue, bookingSources] = await Promise.all([
    row<any>("SELECT COUNT(*) AS total, SUM(is_active) AS active_count FROM portal_tee_slots WHERE club_id = ? AND date BETWEEN ? AND ?", [club.id, from, to]),
    row<any>(`SELECT COUNT(*) AS total,
...
router.get("/portal/tee-times", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  let sql = `SELECT pts.id, pts.date, pts.tee_time AS time, pts.max_players AS total_slots,
       ...
       rl.status AS resale_status,
       rl.price AS resale_price
  ...
  res.json(rows.map(r => ({
    ...
    resale_status: r.resale_status ?? null,
...
router.get("/portal/events/:id/conflicts", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  // Eclectic competitions span the full year and are fed BY other tournaments —
  // they must also never block publishing a regular competition.
...
router.post("/portal/events/:id/resolve-and-publish", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  const club  = getClub(req);
  const evId  = Number(req.params.id);
  const ev    = await row<any>("SELECT id, name, event_date, end_date, restriction, block_full_day FROM golf_events WHERE id = ? AND club_id = ?", [evId, club.id]);
  if (!ev) { res.status(404).json({ message: "Event not found" }); return; }
  ...
  if (!user) {
    ...
    await exec(
      ...
      [club.id, emailClean, hnaClean, membership_type, start_date ?? null, renewal_date ?? null,
       benefits ?? null, Number(prepaid_rounds) || 0, stuClean, memberNo]
    );
...
router.post("/portal/members/import", requireClubAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  for (const r of rows) {
    ...
    let requestedNo: number | null = Number.isFinite(requestedNoRaw) && requestedNoRaw > 0 ? requestedNoRaw : null;
    if (!email) continue;
    ...
    try {
      ...
      // Resolve the member number: use the sheet's number when it's free,
      // otherwise fall back to the next sequential number for the club.
      ...
      if (!user) {
        ...
        await exec(
          ...
          [club.id, email, hnaClean, membership_type, start_date, renewal_date, benefits, prepaid_rounds, stuClean, memberNo]
        );
        pending++;
        if (prepaid_rounds > 0) {
