...
function DeadlineDialog({ round, eventId, onClose, onSaved }: { round: KnockoutRound; eventId: number; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api(`/portal/events/${eventId}/knockout/rounds/${round.id}`, { method: "PUT", body: JSON.stringify({ deadline: deadline || null }) });
...
function ScoreDialog({ match, eventId, onClose, onSaved }: { match: KnockoutMatch; eventId: number; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await api(`/portal/events/${eventId}/knockout/matches/${match.id}`, {
        method: "PUT",
...
function GenerateDialog({ eventId, approvedCount, onClose, onGenerated }: { eventId: number; approvedCount: number; onClose: () => void; onGenerated: () => void }) {
  ...
  const generate = async () => {
    setGenerating(true);
    try {
      await api(`/portal/events/${eventId}/knockout/generate`, {
        method: "POST",
...
export function KnockoutBracketTab({ eventId, eventName, approvedCount, readOnly }: {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await api(`/portal/events/${eventId}/knockout/bracket`);
      setData(d);
    } catch {
      setData(null);
  ...
  const publish = async () => {
    setPublishing(true);
    try {
      const r = await api(`/portal/events/${eventId}/knockout/publish`, { method: "POST" });
